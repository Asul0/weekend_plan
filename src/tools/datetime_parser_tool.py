...
from src.schemas.data_schemas import DateTimeParserToolArgs
...
class TimeRangeResult(BaseModel):
    ...
    is_after_indicator: bool = Field(
        False, description="True, если был распознан маркер времени 'после X', 'от X'."
    )
...
- "до Y часов", "к Y часам": end_hour=Y, end_minute=0 (или YY, если "до Y:YY"), is_range_indicator=true.
- "после X часов", "от X часов": start_hour=X, start_minute=0 (или YY, если "после X:YY"), is_after_indicator=true.
...
async def datetime_parser_tool(
    natural_language_date: str, natural_language_time_qualifier: Optional[str] = None
) -> Optional[Dict[str, Union[datetime, bool, str, None]]]:
    """
    Парсит естественный язык (например, 'завтра' или 'на этих выходных' + 'вечером')
    в точные объекты datetime, используя комбинацию Python и LLM.
    Возвращает словарь с датами начала и окончания.

    Args:
        natural_language_date: Строка, описывающая дату (например, 'завтра', 'на выходных').
        natural_language_time_qualifier: Необязательная строка для уточнения времени
                                         (например, 'вечером', 'после 18:00').
    """
    base_datetime = datetime.now()
    parsed_date_info = python_date_parser(natural_language_date, base_datetime)

    if not parsed_date_info:
        logger.warning(f"Не удалось распознать дату: '{natural_language_date}'")
        return None

    # Теперь обрабатываем время
    if natural_language_time_qualifier:
        llm = get_gigachat_client()
        time_qualifier = natural_language_time_qualifier
        # В качестве базовой даты для парсинга времени используем начало дня извлеченной даты
        base_dt_for_time = parsed_date_info["start_datetime"] or base_datetime
        parsed_time_info = await _parse_time_with_llm_flexible(
            time_qualifier, base_dt_for_time, llm
        )

        # Если время успешно извлечено, применяем его к дате
        if parsed_time_info.start_hour is not None:
            # Обновляем start_datetime, если есть время начала
            parsed_date_info["start_datetime"] = _apply_time_to_datetime(
                base_dt_for_time,
                parsed_time_info.start_hour,
                parsed_time_info.start_minute,
            )

        if parsed_time_info.end_hour is not None:
            # Обновляем end_datetime, если есть время окончания
            # Используем ту же базовую дату, что и для start
            parsed_date_info["end_datetime"] = _apply_time_to_datetime(
                base_dt_for_time, parsed_time_info.end_hour, parsed_time_info.end_minute
            )
            # Убеждаемся, что флаг диапазона установлен, если есть время окончания
            if parsed_time_info.is_range_indicator:
                parsed_date_info["is_range"] = True

    # Конвертируем datetime объекты в ISO-строки для сериализации в JSON
    result_dict = {
        "datetime_iso": (
            parsed_date_info["start_datetime"].isoformat()
            if parsed_date_info["start_datetime"]
            else None
        ),
        "end_datetime_iso": (
            parsed_date_info["end_datetime"].isoformat()
            if parsed_date_info["end_datetime"]
            else None
        ),
        "is_range": parsed_date_info["is_range"],
        "source": "combined",
        "clarification_needed": parsed_date_info["clarification_needed"],
    }
