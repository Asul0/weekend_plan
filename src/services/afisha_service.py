...
async def search_sessions(
    ...
    city_name_for_filter: Optional[str] = None,
) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        logger.error(
            f"Критическая ошибка типа данных для дат! Получено: date_from={type(date_from)}, date_to={type(date_to)}"
        )
        return []
    ...
    if user_creation_type_key == "ANY":
        ...
        current_effective_genres.extend(["юмор", "comedy", "стендап"])
        ...
        current_effective_keywords.extend(
            ["музей", "выставка", "экспозиция", "галерея"]
        )
    else:
        api_type_value = CREATION_TYPES_AFISHA.get(user_creation_type_key)
        api_creation_types_to_query.append(
            api_type_value if api_type_value is not None else None
        )
    ...
    logger.info(
        f"Afisha: Searching sessions: city_id={city_id}, city_name_filter='{city_name_for_filter}', date_range=[{api_date_from_str} to {api_date_to_str}], user_type='{user_creation_type_key}', api_types={api_creation_types_to_query}"
    )
            ...
            while (
                ...
                for creation in creations_on_page:
                    ...
                    if (
                        ...
                        continue
                    if final_effective_keywords and not any(
                ...
                for i, schedule_data in enumerate(schedule_results):
                    ...
                    for schedule_block in schedule_data:
                        ...
                        # --- НАЧАЛО ИСПРАВЛЕННОЙ ЛОГИКИ ФИЛЬТРАЦИИ ---
                        place_address_lower = place_info.get("Address", "").lower()

                        # Мы отбраковываем место, только если в его адресе есть название ДРУГОГО известного города.
                        # Это более надежный подход, чем требовать наличия искомого города в каждой строке.
                        ...
                        is_in_another_city = False

                        if city_name_for_filter:
                            current_city_lower = city_name_for_filter.lower()
                            if current_city_lower in other_cities_to_check:
                                other_cities_to_check.remove(current_city_lower)

                        for other_city in other_cities_to_check:
                            if other_city in place_address_lower:
                                is_in_another_city = True
                                logger.debug(
                                    f"Skipping place '{place_info.get('Name')}' as it appears to be in another city '{other_city}'. Address: '{place_address_lower}'"
                                )
                                break

                        if is_in_another_city:
                            continue  # Переходим к следующему schedule_block
